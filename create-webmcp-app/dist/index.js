...
async function main() {
    ...
    const targetDir = resolve(project.path);
    mkdirSync(targetDir, { recursive: true });
    s.stop('Project directory created');
    // Copy template files
    s.start('Copying template files');
    const templateName = project.template === 'vanilla' ? 'webmcp-template-vanilla' : 'webmcp-template';
    const templateSource = join(__dirname, '..', '..', templateName);
    ...
    if (isInRepo) {
        ...
        copyTemplate(templateSource, targetDir, project.template);
    }
    else {
        // Published package mode - templates are bundled
        const bundledTemplate = join(__dirname, '..', 'templates', templateName);
        if (existsSync(bundledTemplate)) {
            copyTemplate(bundledTemplate, targetDir, project.template);
        }
        else {
...
function copyTemplate(source, target, templateType) {
